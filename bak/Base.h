...
class OneMinute:public BaseData{
		...
		):BaseData(contract,compare_time,current_data,compare_data)
	{
		...
		this->TIME_INTERVAL	= 60;			//时间间隔										 /**********要改的地方**********/
	...
	public:
	    ...
	    {
			//创建对象
			OneMinute* torigin= new OneMinute();
			torigin->contract=(string)(ct->InstrumentID);
			(*(this->tsmd_a))[(string)(ct->InstrumentID)] = torigin;   //注意是this,不是torgin		
			torigin->compare_time = method_TimeToInt_60(ct->UpdateTime-ct->UpdateTime%100);//60	/**********要改的地方**********/
			//更新1分钟标准
		...
		{
			OneMinute * torigin= (*(this->tsmd_a))[(string)(ct->InstrumentID)];            //注意是this
